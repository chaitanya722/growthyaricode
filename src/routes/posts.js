...
const { moderateText, moderateImage, moderateVideo } = require('../middleware/moderation.js');
...
router.post('/', authenticateToken, async (req, res) => {
  try {
    ...
    // 1️⃣ Check text content
    if (moderateText(content) || moderateText(caption)) {
      return res.status(400).json({ success: false, error: 'Inappropriate text detected' });
    }

    // 2️⃣ Check image (if uploaded)
    if (media_url && type === 'image') {
      const isFlagged = await moderateImage(media_url);
      if (isFlagged) {
        return res.status(400).json({ success: false, error: 'Inappropriate image detected' });
      }
    }

    // 3️⃣ Check video (if uploaded)
    if (media_url && type === 'video') {
      const isFlagged = await moderateVideo(media_url);
      if (isFlagged) {
        return res.status(400).json({ success: false, error: 'Inappropriate video detected' });
      }
    }

    // 4️⃣ Save post as usual
    const post = new Post({
