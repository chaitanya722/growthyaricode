...
router.patch('/:id/status', 
  ...
  asyncHandler(async (req: AuthRequest, res) => {
    ...
    const { data: session } = await supabaseAdmin
      .from('sessions')
      .select('id, title, description, duration, scheduled_at, expert_id, client_id, status as current_status')
      .eq('id', id)
      .single();
    ...
    if (status === 'confirmed' && !isExpert) {
      return res.status(403).json({ success: false, error: 'Only expert can confirm sessions' });
    }

    let updateData: any = { status, updated_at: new Date().toISOString() };

    // ✅ Generate Google Meet link when confirmed
    if (status === 'confirmed') {
      try {
        const cal = calendar();
        ...
        updateData.meeting_link = response.data.hangoutLink;
        console.log('✅ Google Meet link generated:', updateData.meeting_link);
    ...
    const { data: updatedSession, error } = await supabaseAdmin
      ...
      .update(updateData)
      .eq('id', id)
      .select(`id, title, status, scheduled_at, meeting_link, expert:expert_id(id, name), client:client_id(id, name)`)
      .single();
